...
from ..motion_control.motion_losses_ours import combined_optical_flow_loss, save_flow_grid_pillow_arrow
        ...
        **kwargs,
        ...
        with torch.no_grad():
            ...
            for j in range(num_samples):
                ...
                x0_pred = self.scheduler.step(noise_pred, timesteps[0], short_latents, **extra_step_kwargs).pred_original_sample

                #obj_loss = torch.mean(x0_pred)
                # with autocast(dtype=torch.float16):
                # #     with torch.no_grad():
                x0_pred = self.decode_latents(x0_pred, stay_on_device = True)[0]

                reference_flows = []
                # reference_flows.append(0.5 + torch.zeros_like(x0_pred))
                
                if(particle_guidance):
                    for file_name in os.listdir(motion_path):
                        if file_name.endswith('.npy'):
                            # Load the .npy file as a NumPy array
                            npy_array = np.load(os.path.join(motion_path, file_name))
                    
                            # Convert the NumPy array to a PyTorch tensor
                            tensor = torch.from_numpy(npy_array).to(device)[0]
                    
                            # Ensure the tensor is the same shape as the current tensor
                            assert tensor.shape == x0_pred.shape, f"Shape mismatch: {tensor.shape} vs {x0_pred.shape}"

                            reference_flows.append(tensor)


                score = combined_optical_flow_loss(x0_pred, reference_flows, brush_mask, 0, sample_loss = True, object_motion = True) 
                # cam_score = combined_optical_flow_loss(x0_pred, [], brush_mask, 0, object_motion = False)

                # score = cam_score + obj_score
                ...
                if (score < best_score):
                    selected_latent = short_latents.clone()
                    best_score = score


            latents = selected_latent
        ...
        # zero_flow_latents = self.scheduler.add_noise(sparse_flow, latents, timesteps[0])
        # latents = zero_flow_latents

        del control

        #sparse_flow.requires_grad = True
        # text_embeddings.requires_grad_(False)
        # stride.requires_grad_(False)
        # latents_img.requires_grad_(False)
        # latents.requires_grad = True
        # brush_mask.requires_grad_(False)
        ...
        # if(controlnet_switch):
        #     nti_obj = NullInversion(self, latents_img, torch.cat([sparse_flow, sparse_mask], dim = 1), text_embeddings)
        # else:
        #     nti_obj = NullInversion(self, latents_img, None, text_embeddings)
        # latents, uncond_embeddings = nti_obj.invert(sparse_flow*0.18215, 'video of a laptop')

        # cond_embeddings = text_embeddings.chunk(2)[1]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # text_embeddings = torch.cat([uncond_embeddings[i].expand(*cond_embeddings.shape), cond_embeddings])

                control = torch.cat([sparse_flow, sparse_mask], dim = 1)    
                # sparse_flow.requires_grad = False
                # sparse_mask.requires_grad = False
                # control.requires_grad = False
                ...
                if(latents.shape[0] == 1 and i < 20 and guidance):
                    #with autocast(dtype = torch.float16):
                    noise_pred = checkpoint.checkpoint(unet_forward, latent_model_input, t, stride, text_embeddings, control_input)
                        # noise_pred = unet_forward(latent_model_input, t=t, stride=stride, 
                        #                           encoder_hidden_states=text_embeddings, control=control_input)

                    with torch.no_grad():
                        old_latents = latents[:, 4:, ...].clone()
                        new_latents = self.scheduler.step(
                            noise_pred, t, old_latents, **extra_step_kwargs
                        ).prev_sample
                        max_grad_norm = 1.0*torch.sqrt(torch.norm(new_latents - old_latents))
                    
                    print('right after unet')
                    ...
                    x0_pred = self.scheduler.step(noise_pred, t, short_latents, **extra_step_kwargs).pred_original_sample
                    ...
                    #x0_pred = x0_pred.requires_grad_()
                    ...
                    # if(i % 1 == 0):     
                    # if(i > 5):
                    obj_loss = combined_optical_flow_loss(
                                x0_pred, 
                                reference_flows, 
                                brush_mask,
                                timestep = i,
                    ...
                    score_loss = combined_optical_flow_loss(
                                x0_pred, 
                                reference_flows, 
                                brush_mask,
                                timestep = i,
                    ...
                    def count_tensors_with_grad():
                        return sum(1 for obj in gc.get_objects() if torch.is_tensor(obj) and obj.grad is not None)
                    

                    #sparse_flow.retain_grad()
                    short_latents.retain_grad()
                    #sparse_flow.grad = None
                    obj_loss.backward(retain_graph=True)
                    # print(x0_pred.grad)
                    # x0_pred_latents_copy.backward(x0_pred_.grad)

                    obj_grad = short_latents.grad.clone()
                    short_latents.grad = None
                    clear_grads()

                    # cam_loss = combined_optical_flow_loss(
                    #             x0_pred, 
                    #             reference_flows, 
                    #             brush_mask,
                    #             timestep = i,
                    ...
                    # cam_grad = short_latents.grad.clone()
                    # short_latents.grad = None
                    # clear_grads()

                    with torch.no_grad():
                        obj_norm = torch.norm(obj_grad)
                        ...
                        #check if obj norm is nan

                        if(torch.isnan(obj_norm)):
                            #set obj grad to zero 
                            obj_grad = torch.zeros_like(obj_grad).to('cuda')


                        
                        # Clip the gradient manually
                        obj_scale_factor = 1.0

                        if obj_norm > max_grad_norm:
                            obj_scale_factor = 1.5*(max_grad_norm / obj_norm)   

                        short_latents = short_latents - obj_scale_factor * obj_grad #* (1 - sparse_mask)


                        # cam_norm = torch.norm(cam_grad)
                        # print('cam grad norm')
                        # print(cam_norm)
                        ...
                        # if cam_norm > max_grad_norm:
                        #     cam_scale_factor = (1.5*max_grad_norm / cam_norm)   

                        # short_latents = short_latents - obj_scale_factor * obj_grad * (1 - sparse_mask) #- cam_scale_factor * cam_grad 
                        # print('cam loss')
                        # print(cam_loss)
                        # print('obj loss')
                        # print(obj_loss)
                        ...
                        # if(cam_loss > 1.0):
                        #     short_latents = short_latents - cam_scale_factor * cam_grad #* (sparse_mask)
                        # if(obj_loss > 1.0):
                        #     short_latents = short_latents - obj_scale_factor * obj_grad #* (1 - sparse_mask)
                        ...
                        # print('clipped grad norm')
                        # print(obj_scale_factor*obj_norm)
                ...
                with torch.no_grad():
                    ...
                    # print('latent step denoiser')
                    # print(torch.norm(latents - old_latents))
                    ...
                    save_flow_grid_pillow_arrow(video[0], os.path.join(output_dir,'debug_vis/combined_timestep' + str(i) + '.gif'))
                    if(i == 0):
                        video = self.decode_latents(sparse_flow*0.18215)
                        video = torch.from_numpy(video)  
                        save_flow_grid_pillow_arrow(video[0], 'debug_vis/sparse_flow' + str(i) + '.gif')
    
                
                torch.cuda.empty_cache()
                gc.collect()
                # with torch.no_grad():
                #     memory_usage = latents.element_size() * latents.nelement()
                #     print(f"Memory usage of latents: {memory_usage / 1024**2:.2f} MB")
        ...
        if output_type == "tensor":
            ...
            save_flow_grid_pillow_arrow(video[0], os.path.join(output_dir,'debug_vis/pre_final_' + str(num_files) +'.gif'))
